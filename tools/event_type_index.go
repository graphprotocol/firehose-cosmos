...
var generateEventTypeIdxCmd = &cobra.Command{
	Use:   "generate-event-type-index {index-url} {irr-index-url} {source-blocks-url} {start-block-num} {stop-block-num}",
	Short: "Generate index files for event types present in blocks",
	Args:  cobra.RangeArgs(4, 5),
	RunE:  generateEventTypeIdxE,
}

func init() {
	generateEventTypeIdxCmd.Flags().Uint64("first-streamable-block", 0, "first streamable block of this chain")
	generateEventTypeIdxCmd.Flags().Uint64("indexes-size", 10000, "size of index bundles that will be created")
	generateEventTypeIdxCmd.Flags().IntSlice("lookup-indexes-sizes", []int{1000000, 100000, 10000, 1000}, "index bundle sizes that we will look for on start to find first unindexed block (should include indexes-size)")
	generateEventTypeIdxCmd.Flags().IntSlice("irreversible-indexes-sizes", []int{10000, 1000}, "size of irreversible indexes that will be used")
	generateEventTypeIdxCmd.Flags().Bool("create-irreversible-indexes", false, "if true, irreversible indexes will also be created")

	Cmd.AddCommand(generateEventTypeIdxCmd)
}

func generateEventTypeIdxE(cmd *cobra.Command, args []string) error {
	...
	idxStart := bstransform.FindNextUnindexed(ctx, uint64(startBlockNum), lookupIdxSizes, transform.EventTypeIndexShortName, indexStore)
	<-done
	...
	t := transform.NewEventTypeIndexer(indexStore, idxSize, startBlockNum)
