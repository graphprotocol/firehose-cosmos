...
var generateEventOriginIdxCmd = &cobra.Command{
	Use:   "generate-event-origin-index {index-url} {irr-index-url} {source-blocks-url} {start-block-num} {stop-block-num}",
	Short: "Generate index files for event origin present in blocks",
	Args:  cobra.RangeArgs(4, 5),
	RunE:  generateEventOriginIdxE,
}

func init() {
	generateEventOriginIdxCmd.Flags().Uint64("first-streamable-block", 0, "first streamable block of this chain")
	generateEventOriginIdxCmd.Flags().Uint64("indexes-size", 10000, "size of index bundles that will be created")
	generateEventOriginIdxCmd.Flags().IntSlice("lookup-indexes-sizes", []int{1000000, 100000, 10000, 1000}, "index bundle sizes that we will look for on start to find first unindexed block (should include indexes-size)")
	generateEventOriginIdxCmd.Flags().IntSlice("irreversible-indexes-sizes", []int{10000, 1000}, "size of irreversible indexes that will be used")
	generateEventOriginIdxCmd.Flags().Bool("create-irreversible-indexes", false, "if true, irreversible indexes will also be created")

	Cmd.AddCommand(generateEventOriginIdxCmd)
}

func generateEventOriginIdxE(cmd *cobra.Command, args []string) error {
	...
	idxStart := bstransform.FindNextUnindexed(ctx, uint64(startBlockNum), lookupIdxSizes, transform.EventOriginIndexShortName, indexStore)
	<-done
	...
	t := transform.NewEventOriginIndexer(indexStore, idxSize, startBlockNum)
